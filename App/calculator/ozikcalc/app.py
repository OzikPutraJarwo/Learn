import toga
from toga.style import Pack
from toga.style.pack import COLUMN, ROW

class Calculator(toga.App):
    def startup(self):
        self.main_window = toga.MainWindow(title=self.formal_name)
        
        # Container for layout
        box = toga.Box(style=Pack(direction=COLUMN, padding=10))

        # Display widget
        self.result = toga.TextInput(readonly=True, style=Pack(flex=1))

        # Create a grid of buttons
        grid = [
            ['7', '8', '9', '/'],
            ['4', '5', '6', '*'],
            ['1', '2', '3', '-'],
            ['0', '.', '=', '+']
        ]

        # Create a box for each row of buttons
        for row in grid:
            row_box = toga.Box(style=Pack(direction=ROW))
            for label in row:
                button = toga.Button(label, on_press=self.on_button_press, style=Pack(flex=1, padding=5))
                row_box.add(button)
            box.add(row_box)

        box.add(self.result)
        self.main_window.content = box
        self.main_window.show()

        # Find the 'Help > About Ozik's Calculator' menu item and update its handler
        about_command = toga.Command(self.about, 'About Ozik\'s Calculator', tooltip='About this application')
        self.commands.add(about_command)

    def on_button_press(self, widget):
        if widget.text == '=':
            try:
                self.result.value = str(eval(self.result.value))
            except:
                self.result.value = 'Error'
        else:
            if self.result.value == 'Error':
                self.result.value = ''
            self.result.value += widget.text

    def about(self, widget):
        # Content of the About dialog
        about_box = toga.Box(style=Pack(direction=COLUMN, padding=10))
        about_label = toga.Label(
            'Ozik\'s Calculator\n\nA simple calculator app built with Toga.',
            style=Pack(padding=5)
        )
        about_box.add(about_label)

        about_window = toga.Window(title="About")
        about_window.content = about_box
        about_window.show()

def main():
    return Calculator('Ozik\'s Calculator', 'org.example.ozikcalc')

if __name__ == '__main__':
    main().main_loop()
